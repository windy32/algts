...
GuiApplication::GuiApplication(
    ...
    : CoreApplication(localAddrs, serverDaemonAddr, serverDaemonPort)
{
}

void GuiApplication::exec(ScenarioEx *s)
{
    ...
    // Generate regular trace
    LOG_INFO("Generating regular trace...");
    ...
    for(it = tasks.begin(); it != tasks.end(); ++it)
    {
        ...
        {
            // insert user
            if( !s->getTraces().contains(username))
            {
                s->getTraces().insert(username, QVector<RegularTraceItem>());
            }
            
            // insert regular trace item
            RegularTraceItem item;
            clients[index++]->generateRegularTrace(item, s->length());
            s->getTraces()[username].append(item);
        }
    }

    LOG_DEBUG("End of GuiApplication::exec");
}
